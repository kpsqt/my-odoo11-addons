...
class BackwardsReader:
    """
    from http://code.activestate.com/recipes/120686-read-a-text-file-backwards/
    ...
    def readline(self):
        while len(self.data) == 1 and ((self.blkcount * self.blksize) < self.size):
            ...
            try:
                ...
                self.data = (self.f.read(self.blksize).decode('utf8') + line).split('\n')
                ...
                self.data = (self.f.read(self.size - (self.blksize * (self.blkcount - 1))).decode('utf8') + line).split('\n')
    ...
    def __init__(self, file, blksize=4096):
        ...
        self.data = (self.f.read(self.blksize).decode('utf8')).split('\n')
